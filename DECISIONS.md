...
## [2026-02-01] PENDING — Velocity Threshold Enforcement Design Decision
**Context:** Session 34 discovered that landing speed thresholds (V<40, H<25) have been dead code since the initial commit. SpriteKit zeros velocities during collision resolution before the `didBegin(contact:)` callback fires. Every build through Build 18 accepted landings at any speed as long as rotation was under 0.05 rad. Build 19 was the first to enforce speed thresholds (using pre-contact tracking), but the tracked values are captured pre-thrust, making them too strict. The HUD also displays wrong threshold values (V<50, H<30) since its creation. All prior player scores were achieved under the "speed doesn't matter" regime.
**Options considered:** (A) Revert to Build 18 behavior — speed thresholds remain dead code, display-only inconsistency returns. (B) Move tracking to post-thrust position and keep V<40/H<25 — first-ever enforcement, game becomes harder than all prior builds. (C) Move tracking to post-thrust and raise thresholds (V<60, H<40) — enforcement but generous. (D) Split sources (post-collision for threshold, tracked for display) — inconsistency between display and enforcement.
**Decision:** **PENDING — requires developer/designer input.** Full analysis in `Docs/DIAGNOSTIC_velocity_thresholds.md`.
**Why:** This is a game design decision, not a bug fix. The game was playable and enjoyable without speed enforcement. Enforcing speed changes the fundamental challenge (rotation-only → rotation+speed). Existing high scores may become unreproducible.
**Consequences:** Depends on chosen option. All options require fixing the HUD threshold mismatch regardless.
